...
(async()=>{const {server,base}=await require('./browser-test-server.cjs')();const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 await page.addInitScript(()=>{try{sessionStorage.setItem('alephonIntroSeen','1');}catch(_){/* Embedded PDF contexts may not expose storage. */}});
 ...
 for(const [width,height] of [[1672,941],[1366,768],[768,1024],[390,844],[320,740]]){
  ...
  assert.equal(await page.locator('#about').count(),0);assert.equal(await page.locator('#contact').evaluate(e=>getComputedStyle(e).display),'none');
  ...
  assert.equal(await page.locator('#about').count(),0);assert.notEqual(await page.locator('#contact').evaluate(e=>getComputedStyle(e).display),'none');
  assert.ok(await page.evaluate(()=>{
   const viewer=document.querySelector('.portfolio-layout'),footer=document.querySelector('#contact');
   return viewer.nextElementSibling===footer && Math.abs(viewer.getBoundingClientRect().bottom-footer.getBoundingClientRect().top)<1;
  }),'footer immediately follows viewer with no gap');
  assert.deepEqual(await header(),baseline,'viewer header matches home');
  await page.locator('[data-home]').click();await page.waitForFunction(()=>document.documentElement.dataset.view==='landing');assert.equal(await page.locator('#about').count(),0);
 ...
 await page.goto(base+'#about');assert.equal(await page.locator('html').getAttribute('data-view'),'viewer');assert.equal(await page.locator('#about').count(),0);
 assert.ok(page.url().endsWith('#portfolio-viewer'),'legacy About bookmark resolves to viewer');
 await page.evaluate(()=>location.hash='about');await page.waitForURL('**/#portfolio-viewer');
 assert.deepEqual(errors,[]);console.log('PASS: editorial facts/contacts, single-screen desktop, natural mobile flow, keyboard underline, removed About and preserved footer, Resume footer, identical shared header geometry at five viewport sizes, legacy About redirect.');
